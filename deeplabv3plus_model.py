...
def Deeplabv3(pretrained_weights=None, input_shape=(512,512,3), num_classes=4,
			  alpha=1., backbone="xception", downsample_factor=16,learning_rate=1e-5):
	...
	#   将加强特征边上采样
	# -----------------------------------------#
	x = Lambda(lambda xx: tf.compat.v1.image.resize_bilinear(xx, skip_size[1:3], align_corners=True))(x)
	# ----------------------------------#
	#   浅层特征边
	# ----------------------------------#
